import os

# Name of the directory containing data files
DATA_DIR: str = "data"
# Name of the file containing course data
# COURSES_FILE_NAME: str = os.path.join(DATA_DIR, "cs-courses.json")
COURSES_FILE_NAME: str = os.path.join(DATA_DIR, "courses.json")
# Name of the file containing degree data
# DEGREES_FILE_NAME: str = os.path.join(DATA_DIR, "cs-degree.json")
# DEGREES_FILE_NAME: str = os.path.join(DATA_DIR, "cs-bs-degree.json")
DEGREES_FILE_NAME: str = os.path.join(DATA_DIR, "degrees.json")
# DEGREES_FILE_NAME: str = os.path.join(DATA_DIR, "altered-degrees.json")
UNSOLVABLE_DEGREES_FILE_NAME: str = os.path.join(DATA_DIR, "unsolvable-degrees.json")
